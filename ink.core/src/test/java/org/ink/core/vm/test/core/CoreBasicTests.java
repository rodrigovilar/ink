...
public class CoreBasicTests extends TestCase {
	...
	private void ink(Class<?> base) {
		...
		if (!zuperImplName.equals(zuperImplNameExp)) {
			System.out.println(baseImplName + ": Expected to extend " + zuperImplNameExp + " but extending instead " + zuperImplName);
		}
		...
		if (!isInkObject) {
			...
			if (!zuperStateName.equals(zuperStateNameExp)) {
				System.out.println("Expected " + zuperStateNameExp + " but found " + zuperState);
			}
