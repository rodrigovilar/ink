...
public class GenerateJavaOperation extends InkEditorOperation{
	...
	protected boolean execute(InkObject o, Shell shell, IDocument doc, IFile file) throws Exception {
		if(o.reflect().isClass()){
			...
			if(cm.isStruct()){
				showDialog(shell, "The Ink class " + cm.getShortId() +" is a Struct and can't have Java Behavior/Interface classes.",
				...
				if(hasBehavior && hasInterface){
					showDialog(shell, "Behavior and Interface classes already exist. Nothing to generate.",
							MessageDialog.INFORMATION, new String[] {IDialogConstants.OK_LABEL});
				}else if(hasInterface){
					int result = showDialog(shell, "Generate behavior class ?",
					...
					int result = showDialog(shell, "Generate interface class ?",
							MessageDialog.QUESTION, new String[] {IDialogConstants.OK_LABEL, IDialogConstants.CANCEL_LABEL});
					if(result==0){
						jm = JavaMapping.Behavior_Interface;
					...
					int result = showDialog(shell, "Generate java classes ?",
							MessageDialog.QUESTION, new String[] {"Generate Behavior","Generate Interface","Generate Both", IDialogConstants.CANCEL_LABEL});
					if(result==0){
	...
	private int showDialog(Shell shell, String msg, int msgType, String[] buttons) {
		MessageDialog dialog = new MessageDialog(shell,"Generate Java Classes Dialog",
				null,msg,msgType, buttons, 0) {
			@Override
