...
InkObjectImpl<S> implements ActiveOffers {
	...
	public A_SpecialOffer getBestOffer(A_Subscription subscription) {
		...
		if (bestOffer != null) {
			result = new SpecialOffer(bestOffer.getPromotionalPrice(subscription), bestOffer.getFreeIssues(subscription), bestOffer.getPromotionalMessage(subscription), bestOffer);
		}
		else {
			result = new SpecialOffer(subscription.getPriceForSubscriptionPeriod(), 0, "Sorry, No special offer for you.", null);
		}
